...
parameters AS (
	SELECT
		...
		DATE '2026-06-22' AS end_date,
		7::INTEGER AS lookback_days,
		'connected' AS ready_status,
		'registered' AS review_status,
		'pending' AS blocked_status,
		'orders_4' AS order_bucket,
		'payments_1' AS payment_bucket,
...
raw_order_activity AS (
	SELECT
		...
		CASE
			WHEN no.NO_O_ID IS NOT NULL THEN 'registered'
			WHEN o.O_CARRIER_ID IS NULL THEN 'pending'
			ELSE 'connected'
		END AS order_status
	FROM dl.OORDER o
	...
	INNER JOIN date_spine ds
		ON CAST(o.O_ENTRY_D AS DATE) BETWEEN ds.report_as_of_date - INTERVAL 7 DAY
...
history_activity AS (
	...
	FROM dl.HISTORY h
	INNER JOIN date_spine ds
		ON CAST(h.H_DATE AS DATE) BETWEEN ds.report_as_of_date - INTERVAL 7 DAY
		AND ds.report_as_of_date
	GROUP BY
		ds.report_as_of_date,
		h.H_C_W_ID,
...
unpivoted AS (
	...
	SELECT
		...
		ec.district_name_norm,
		ec.customer_last_norm,
		'activity' AS metric_family,
		CASE
			WHEN ec.payment_amount > ec.balance_amount THEN 'connected'
			WHEN ec.payment_events > 0 THEN 'registered'
			ELSE 'pending'
	...
	SELECT
		...
		'supply' AS metric_family,
		CASE
			WHEN ls.stock_quantity > 67 THEN 'connected'
			WHEN ls.stock_quantity > 47 THEN 'registered'
			ELSE 'pending'
		END AS metric_status,
		ls.item_id,
		ls.stock_quantity,
...
SELECT
	...
	CASE
		WHEN metric_amount >= 1200.0 THEN 'large'
		WHEN metric_amount >= 12.0 THEN 'medium'
		ELSE 'small'
	END AS metric_size
FROM final_enriched
ORDER BY
