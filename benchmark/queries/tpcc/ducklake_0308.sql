-- {"complexity": "extreme", "is_incremental": true, "ducklake": true, "openivm_verified": true, "operators": "DUCKLAKE,CTE,UNNEST,CONSTANT,INNER_JOIN,AGGREGATE,HAVING"}
WITH
parameters AS (
	SELECT
...
bucket_arrays AS (
	SELECT 0 AS bucket_id, [1, 2, 3, 4, 5] AS item_ids
	UNION ALL SELECT 1 AS bucket_id, [6, 7, 8, 9, 10] AS item_ids
	UNION ALL SELECT 2 AS bucket_id, [11, 12, 13, 14, 15] AS item_ids
	UNION ALL SELECT 3 AS bucket_id, [16, 17, 18, 19, 20] AS item_ids

),
unnested_bucket_items AS (
	SELECT
		ba.bucket_id,
		UNNEST(ba.item_ids) AS item_id
	FROM bucket_arrays ba
...
unnest_joined AS (
	...
	JOIN unnested_bucket_items ubi
		ON ubi.item_id = ff.item_id
...
unnest_rollup AS (
	SELECT
		...
		uj.bucket_id,
		uj.credit_code,
		uj.fulfillment_state,
		COUNT(*) AS bucket_rows,
		SUM(uj.extended_amount) AS bucket_amount,
		MIN(uj.stock_quantity) AS min_bucket_stock,
		MAX(uj.stock_quantity) AS max_bucket_stock
	FROM unnest_joined uj
	GROUP BY
		uj.warehouse_id,
		uj.district_id,
		uj.bucket_id,
...
SELECT
	...
	max_bucket_stock
FROM unnest_rollup
ORDER BY warehouse_id, district_id, bucket_id, credit_code, fulfillment_state
