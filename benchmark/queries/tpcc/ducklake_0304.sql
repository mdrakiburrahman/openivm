-- {"complexity": "extreme", "is_incremental": true, "ducklake": true, "openivm_verified": true, "operators": "DUCKLAKE,CTE,SCALAR_SUBQUERY,SINGLE_JOIN,DELIM_JOIN,AGGREGATE,HAVING"}
WITH
parameters AS (
	SELECT
...
scalar_projection AS (
	SELECT
		...
		ff.extended_amount,
		ff.customer_balance,
		(SELECT COUNT(*) FROM history_final hx WHERE hx.warehouse_id = ff.warehouse_id AND hx.district_id = ff.district_id AND hx.customer_id = ff.customer_id) AS correlated_history_rows,
		(SELECT COALESCE(MAX(sx.stock_quantity), 0) FROM stock_final sx WHERE sx.warehouse_id = ff.supply_warehouse_id AND sx.item_id = ff.item_id) AS correlated_stock_ceiling,
		(SELECT COALESCE(SUM(olx.line_amount), 0.0) FROM order_line_final olx WHERE olx.warehouse_id = ff.warehouse_id AND olx.district_id = ff.district_id AND olx.order_id = ff.order_id) AS correlated_order_amount
...
scalar_grouped AS (
	SELECT
		...
		SUM(sp.extended_amount + sp.correlated_order_amount) AS scalar_amount,
		SUM(sp.correlated_history_rows) AS history_row_sum,
		MAX(sp.correlated_stock_ceiling) AS stock_ceiling,
		MIN(sp.customer_balance) AS min_balance
	FROM scalar_projection sp
	GROUP BY
		sp.warehouse_id,
		sp.district_id,
		sp.credit_code
