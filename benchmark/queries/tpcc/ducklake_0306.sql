-- {"complexity": "extreme", "is_incremental": true, "ducklake": true, "openivm_verified": true, "operators": "DUCKLAKE,CTE,UNION,AGGREGATE,HAVING,NESTED_AGGREGATE"}
WITH
parameters AS (
	SELECT
...
sales_branch AS (
	SELECT
		ff.warehouse_id,
		ff.district_id,
		'sales' AS branch_name,
		ff.credit_code,
		COUNT(*) AS branch_rows,
		SUM(ff.extended_amount) AS branch_amount,
		MIN(ff.extended_amount) AS branch_min,
		MAX(ff.extended_amount) AS branch_max
...
payment_branch AS (
	SELECT
		hf.warehouse_id,
		hf.district_id,
		'payments' AS branch_name,
		'GC' AS credit_code,
		COUNT(*) AS branch_rows,
		SUM(hf.history_amount) AS branch_amount,
		MIN(hf.history_amount) AS branch_min,
		MAX(hf.history_amount) AS branch_max
	FROM history_final hf
	GROUP BY
		hf.warehouse_id,
		hf.district_id
...
stock_branch AS (
	SELECT
		sf.warehouse_id,
		1 AS district_id,
		'stock' AS branch_name,
		'GC' AS credit_code,
		COUNT(*) AS branch_rows,
		SUM(CAST(sf.stock_quantity AS DOUBLE)) AS branch_amount,
		MIN(CAST(sf.stock_quantity AS DOUBLE)) AS branch_min,
		MAX(CAST(sf.stock_quantity AS DOUBLE)) AS branch_max
	FROM stock_final sf
	GROUP BY
		sf.warehouse_id
...
branch_rollup AS (
	SELECT
		...
		bu.branch_name,
		bu.credit_code,
		SUM(bu.branch_rows) AS total_rows,
		SUM(bu.branch_amount) AS total_amount,
		MIN(bu.branch_min) AS min_branch_value,
		MAX(bu.branch_max) AS max_branch_value
	FROM branch_union bu
	GROUP BY
		bu.warehouse_id,
		bu.district_id,
		bu.branch_name,
		bu.credit_code
	HAVING SUM(bu.branch_rows) >= 0
...
SELECT
	...
	max_branch_value
FROM branch_rollup
ORDER BY warehouse_id, district_id, branch_name, credit_code
