-- {"complexity": "extreme", "is_incremental": true, "ducklake": true, "openivm_verified": true, "operators": "DUCKLAKE,CTE,MATERIALIZED_CTE,LATERAL,TABLE_FUNCTION,DELIM_JOIN,WINDOW,AGGREGATE,HAVING"}
WITH
parameters AS (
	SELECT
...
lateral_seed AS MATERIALIZED (
	...
	FROM fact_final ff
	WHERE ff.line_number BETWEEN 1 AND 5
...
lateral_expanded AS (
	SELECT
		...
		ls.item_id,
		ls.credit_code,
		ls.fulfillment_state,
		ls.account_state,
		ls.extended_amount,
		ls.stock_quantity,
		ls.total_history_amount,
		CAST(slots.slot AS INTEGER) AS line_slot,
		ls.extended_amount * CAST(slots.slot AS DECIMAL(18, 4)) AS slot_amount,
		ls.stock_quantity + CAST(slots.slot AS INTEGER) AS slot_stock
	FROM lateral_seed ls
	CROSS JOIN LATERAL (
		SELECT generate_series AS slot
		FROM generate_series(1, LEAST(ls.line_number, 3))
	) slots
...
lateral_window_1 AS (
	SELECT
		...
		le.item_id,
		le.credit_code,
		le.fulfillment_state,
		le.account_state,
		le.line_slot,
		le.slot_amount,
		le.slot_stock,
		ROW_NUMBER() OVER (
			PARTITION BY le.warehouse_id, le.district_id, le.customer_id
			ORDER BY le.line_slot DESC, le.order_id ASC, le.item_id ASC
		) AS lateral_rank,
		SUM(le.slot_amount) OVER (
			PARTITION BY le.warehouse_id, le.district_id, le.customer_id
		) AS lateral_customer_amount
	FROM lateral_expanded le
...
lateral_window_2 AS (
	SELECT
		...
		lw1.item_id,
		lw1.credit_code,
		lw1.fulfillment_state,
		lw1.account_state,
		lw1.line_slot,
		lw1.slot_amount,
		lw1.slot_stock,
		lw1.lateral_rank,
		lw1.lateral_customer_amount,
		DENSE_RANK() OVER (
			PARTITION BY lw1.warehouse_id, lw1.credit_code
			ORDER BY lw1.lateral_customer_amount DESC, lw1.customer_id ASC
		) AS lateral_credit_rank,
		COUNT(*) OVER (
			PARTITION BY lw1.warehouse_id, lw1.district_id, lw1.line_slot
		) AS slot_peer_rows
	FROM lateral_window_1 lw1
...
lateral_rollup AS (
	SELECT
		...
		line_slot,
		COUNT(*) AS slot_rows,
		SUM(slot_amount) AS slot_total_amount,
		SUM(slot_stock) AS slot_total_stock,
		MAX(lateral_rank) AS max_lateral_rank,
		MAX(lateral_credit_rank) AS max_lateral_credit_rank,
		MAX(slot_peer_rows) AS max_slot_peer_rows,
		COUNT(*) FILTER (WHERE lateral_rank <= 2) AS top_lateral_rows
	FROM lateral_window_2
	GROUP BY
		warehouse_id,
		district_id,
		credit_code,
		...
		account_state,
		line_slot
	HAVING SUM(slot_amount) IS NOT NULL
...
SELECT
	...
	slot_total_stock,
	max_lateral_rank,
	max_lateral_credit_rank,
	max_slot_peer_rows,
	top_lateral_rows
FROM lateral_rollup
ORDER BY warehouse_id, district_id, credit_code, fulfillment_state, account_state, line_slot
