...
unpivoted AS (
	...
	SELECT
		...
		CASE
			WHEN ls.stock_quantity > 62 THEN 'connected'
			WHEN ls.stock_quantity > 42 THEN 'registered'
			ELSE 'pending'
		END AS metric_status,
		ls.item_id,
		ls.stock_quantity,
...
SELECT
	...
	CASE
		WHEN metric_amount >= 700.0 THEN 'large'
		WHEN metric_amount >= 7.0 THEN 'medium'
		ELSE 'small'
	END AS metric_size
FROM final_enriched
ORDER BY
