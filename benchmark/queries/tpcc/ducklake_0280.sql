...
parameters AS (
	SELECT
		...
		'stock_5' AS stock_bucket
),
date_spine AS (
		SELECT DATE '2026-06-14' AS report_as_of_date
...
unpivoted AS (
	...
	SELECT
		...
		CASE
			WHEN ls.stock_quantity > 67 THEN 'balanced'
			WHEN ls.stock_quantity > 47 THEN 'credit'
			ELSE 'debt'
		END AS metric_status,
		ls.item_id,
		ls.stock_quantity,
...
SELECT
	...
	CASE
		WHEN metric_amount >= 800.0 THEN 'large'
		WHEN metric_amount >= 8.0 THEN 'medium'
		ELSE 'small'
	END AS metric_size
FROM final_enriched
ORDER BY
