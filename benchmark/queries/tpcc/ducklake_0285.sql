...
parameters AS (
	SELECT
		...
		'orders_6' AS order_bucket,
		'payments_4' AS payment_bucket,
		'stock_4' AS stock_bucket
),
date_spine AS (
...
unpivoted AS (
	...
	SELECT
		...
		CASE
			WHEN ls.stock_quantity > 55 THEN 'balanced'
			WHEN ls.stock_quantity > 35 THEN 'credit'
			ELSE 'debt'
		END AS metric_status,
		ls.item_id,
		ls.stock_quantity,
...
SELECT
	...
	CASE
		WHEN metric_amount >= 1300.0 THEN 'large'
		WHEN metric_amount >= 13.0 THEN 'medium'
		ELSE 'small'
	END AS metric_size
FROM final_enriched
ORDER BY
