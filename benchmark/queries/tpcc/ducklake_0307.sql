-- {"complexity": "extreme", "is_incremental": true, "ducklake": true, "openivm_verified": true, "operators": "DUCKLAKE,CTE,AGGREGATE,COUNT_DISTINCT,ARG_MINMAX,MINMAX,HAVING"}
WITH
parameters AS (
	SELECT
...
distinct_minmax_grouped AS (
	SELECT
		dmi.warehouse_id,
		dmi.district_id,
		dmi.credit_code,
		dmi.fulfillment_state,
		COUNT(*) AS row_count,
		COUNT(DISTINCT dmi.customer_id) AS distinct_customers,
		COUNT(DISTINCT dmi.item_id) AS distinct_items,
		SUM(dmi.extended_amount) AS amount_sum,
		MIN(dmi.customer_balance) AS min_balance,
		MAX(dmi.customer_balance) AS max_balance,
		MIN(dmi.stock_quantity) AS min_stock_quantity,
		MAX(dmi.stock_quantity) AS max_stock_quantity,
		ARG_MAX(dmi.item_id, dmi.extended_amount + CAST(dmi.line_number AS DOUBLE) * 0.001 + CAST(dmi.order_id AS DOUBLE) * 0.000001) AS top_amount_item,
		ARG_MIN(dmi.order_id, dmi.customer_balance + CAST(dmi.customer_id AS DOUBLE) * 0.001 + CAST(dmi.order_id AS DOUBLE) * 0.000001) AS low_balance_order
	FROM distinct_minmax_input dmi
	GROUP BY
		dmi.warehouse_id,
		dmi.district_id,
		dmi.credit_code,
