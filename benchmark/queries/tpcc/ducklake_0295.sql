...
parameters AS (
	SELECT
		...
		'orders_2' AS order_bucket,
		'payments_4' AS payment_bucket,
		'stock_2' AS stock_bucket
),
date_spine AS (
		SELECT DATE '2026-06-17' AS report_as_of_date
...
unpivoted AS (
	...
	SELECT
		...
		CASE
			WHEN ls.stock_quantity > 65 THEN 'balanced'
			WHEN ls.stock_quantity > 45 THEN 'credit'
			ELSE 'debt'
		END AS metric_status,
		ls.item_id,
		ls.stock_quantity,
...
SELECT
	...
	CASE
		WHEN metric_amount >= 1000.0 THEN 'large'
		WHEN metric_amount >= 10.0 THEN 'medium'
		ELSE 'small'
	END AS metric_size
FROM final_enriched
ORDER BY
