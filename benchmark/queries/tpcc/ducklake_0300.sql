...
parameters AS (
	SELECT
		...
		'orders_0' AS order_bucket,
		'payments_4' AS payment_bucket,
		'stock_1' AS stock_bucket
),
date_spine AS (
		SELECT DATE '2026-06-18' AS report_as_of_date
...
unpivoted AS (
	...
	SELECT
		...
		CASE
			WHEN ls.stock_quantity > 70 THEN 'balanced'
			WHEN ls.stock_quantity > 50 THEN 'credit'
			ELSE 'debt'
		END AS metric_status,
		ls.item_id,
		ls.stock_quantity,
...
SELECT
	...
	CASE
		WHEN metric_amount >= 1500.0 THEN 'large'
		WHEN metric_amount >= 15.0 THEN 'medium'
		ELSE 'small'
	END AS metric_size
FROM final_enriched
ORDER BY
