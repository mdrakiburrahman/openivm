-- {"complexity": "extreme", "is_incremental": true, "ducklake": true, "openivm_verified": true, "operators": "DUCKLAKE,CTE,WINDOW,INNER_JOIN,LEFT_JOIN,FILTER"}
WITH
parameters AS (
	SELECT
...
window_ranked AS (
	SELECT
		...
		wi.item_id,
		wi.credit_code,
		wi.fulfillment_state,
		wi.extended_amount,
		wi.customer_balance,
		wi.stock_quantity,
		wi.account_state,
		ROW_NUMBER() OVER (PARTITION BY wi.warehouse_id, wi.district_id ORDER BY wi.extended_amount DESC, wi.order_id ASC, wi.line_number ASC) AS district_amount_rank,
		DENSE_RANK() OVER (PARTITION BY wi.warehouse_id, wi.credit_code ORDER BY wi.customer_balance DESC, wi.customer_id ASC) AS credit_balance_rank,
		SUM(wi.extended_amount) OVER (PARTITION BY wi.warehouse_id, wi.district_id, wi.customer_id) AS customer_window_amount,
		MAX(wi.stock_quantity) OVER (PARTITION BY wi.warehouse_id, wi.item_id) AS item_window_stock
...
window_filtered AS (
	SELECT
		...
		stock_quantity,
		account_state,
		district_amount_rank,
		credit_balance_rank,
		customer_window_amount,
		item_window_stock
	FROM window_ranked
	WHERE district_amount_rank <= 4 OR credit_balance_rank <= 6
