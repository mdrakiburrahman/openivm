...
unpivoted AS (
	...
	SELECT
		...
		CASE
			WHEN ls.stock_quantity > 69 THEN 'hot'
			WHEN ls.stock_quantity > 49 THEN 'warm'
			ELSE 'cold'
		END AS metric_status,
		ls.item_id,
		ls.stock_quantity,
...
SELECT
	...
	CASE
		WHEN metric_amount >= 1400.0 THEN 'large'
		WHEN metric_amount >= 14.0 THEN 'medium'
		ELSE 'small'
	END AS metric_size
FROM final_enriched
ORDER BY
