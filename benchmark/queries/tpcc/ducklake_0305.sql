-- {"complexity": "extreme", "is_incremental": true, "ducklake": true, "openivm_verified": true, "operators": "DUCKLAKE,CTE,FULL_OUTER_JOIN,AGGREGATE,HAVING,MINMAX"}
WITH
parameters AS (
	SELECT
...
customer_side_summary AS (
	SELECT
		cdj.warehouse_id,
		cdj.district_id,
		cdj.credit_code,
		COUNT(*) AS customer_rows,
		SUM(cdj.customer_balance) AS customer_balance_sum,
		MIN(cdj.customer_balance) AS min_customer_balance,
		MAX(cdj.customer_balance) AS max_customer_balance
	FROM customer_district_joined cdj
	GROUP BY
		cdj.warehouse_id,
		cdj.district_id,
		cdj.credit_code
...
order_side_summary AS (
	SELECT
		ff.warehouse_id,
		ff.district_id,
		ff.credit_code,
		COUNT(*) AS order_line_rows,
		SUM(ff.extended_amount) AS order_amount_sum,
		MIN(ff.extended_amount) AS min_order_amount,
		MAX(ff.extended_amount) AS max_order_amount
	FROM fact_final ff
	GROUP BY
		ff.warehouse_id,
		ff.district_id,
...
full_joined_summary AS (
	SELECT
		COALESCE(css.warehouse_id, oss.warehouse_id) AS warehouse_id,
		COALESCE(css.district_id, oss.district_id) AS district_id,
		COALESCE(css.credit_code, oss.credit_code) AS credit_code,
		COALESCE(css.customer_rows, 0) AS customer_rows,
		COALESCE(oss.order_line_rows, 0) AS order_line_rows,
		COALESCE(css.customer_balance_sum, 0.0) AS customer_balance_sum,
		COALESCE(oss.order_amount_sum, 0.0) AS order_amount_sum,
		css.min_customer_balance,
		css.max_customer_balance,
		oss.min_order_amount,
		oss.max_order_amount
	FROM customer_side_summary css
	FULL OUTER JOIN order_side_summary oss
		ON oss.warehouse_id = css.warehouse_id
		AND oss.district_id = css.district_id
		AND oss.credit_code = css.credit_code

),
full_outer_rollup AS (
	SELECT
		fjs.warehouse_id,
		fjs.district_id,
		fjs.credit_code,
		SUM(fjs.customer_rows) AS customer_rows,
		SUM(fjs.order_line_rows) AS order_line_rows,
		SUM(fjs.customer_balance_sum) AS customer_balance_sum,
		SUM(fjs.order_amount_sum) AS order_amount_sum,
		MIN(fjs.min_customer_balance) AS min_customer_balance,
		MAX(fjs.max_customer_balance) AS max_customer_balance,
		MIN(fjs.min_order_amount) AS min_order_amount,
		MAX(fjs.max_order_amount) AS max_order_amount
	FROM full_joined_summary fjs
	GROUP BY
		fjs.warehouse_id,
		fjs.district_id,
		fjs.credit_code
...
SELECT
	...
	customer_rows,
	order_line_rows,
	customer_balance_sum,
	order_amount_sum,
	min_customer_balance,
	max_customer_balance,
	min_order_amount,
	max_order_amount
FROM full_outer_rollup
ORDER BY warehouse_id, district_id, credit_code
