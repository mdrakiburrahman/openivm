...
parameters AS (
	SELECT
		DATE '2026-06-12' AS start_date,
		DATE '2026-06-23' AS end_date,
...
date_spine AS (
		SELECT DATE '2026-06-12' AS report_as_of_date
...
unpivoted AS (
	...
	SELECT
		...
		CASE
			WHEN ls.stock_quantity > 57 THEN 'balanced'
			WHEN ls.stock_quantity > 37 THEN 'credit'
			ELSE 'debt'
		END AS metric_status,
		ls.item_id,
		ls.stock_quantity,
...
SELECT
	...
	CASE
		WHEN metric_amount >= 1100.0 THEN 'large'
		WHEN metric_amount >= 11.0 THEN 'medium'
		ELSE 'small'
	END AS metric_size
FROM final_enriched
ORDER BY
