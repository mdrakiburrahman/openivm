-- {"complexity": "extreme", "is_incremental": true, "ducklake": true, "openivm_verified": true, "operators": "DUCKLAKE,CTE,SEMI_JOIN,ANTI_JOIN,DELIM_JOIN,AGGREGATE,HAVING,COUNT_DISTINCT"}
WITH
parameters AS (
	SELECT
...
semi_anti_filtered AS (
	...
	WHERE EXISTS (
		SELECT 1
		FROM stock_final sx
		WHERE sx.warehouse_id = ff.supply_warehouse_id
		  AND sx.item_id = ff.item_id
		  AND sx.stock_quantity >= ff.line_quantity
	)
	AND NOT EXISTS (
		SELECT 1
		FROM new_order_final nox
		WHERE nox.warehouse_id = ff.warehouse_id
		  AND nox.district_id = ff.district_id
		  AND nox.order_id = ff.order_id
		  AND ff.fulfillment_state = 'closed'
	)

),
semi_anti_grouped AS (
	SELECT
		saf.warehouse_id,
		saf.district_id,
		saf.credit_code,
		saf.account_state,
		COUNT(*) AS kept_rows,
		COUNT(DISTINCT saf.customer_id) AS distinct_customers,
		SUM(saf.extended_amount) AS kept_amount,
		MIN(saf.customer_balance) AS min_balance,
		MAX(saf.stock_quantity) AS max_stock_quantity
	FROM semi_anti_filtered saf
	GROUP BY
		saf.warehouse_id,
		saf.district_id,
		saf.credit_code,
...
SELECT
	...
	min_balance,
	max_stock_quantity
FROM semi_anti_grouped
ORDER BY warehouse_id, district_id, credit_code, account_state
