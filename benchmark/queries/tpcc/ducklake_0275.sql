...
date_spine AS (
		SELECT DATE '2026-06-13' AS report_as_of_date
...
unpivoted AS (
	...
	SELECT
		...
		CASE
			WHEN ls.stock_quantity > 62 THEN 'balanced'
			WHEN ls.stock_quantity > 42 THEN 'credit'
			ELSE 'debt'
		END AS metric_status,
		ls.item_id,
		ls.stock_quantity,
...
SELECT
	...
	CASE
		WHEN metric_amount >= 1600.0 THEN 'large'
		WHEN metric_amount >= 16.0 THEN 'medium'
		ELSE 'small'
	END AS metric_size
FROM final_enriched
ORDER BY
