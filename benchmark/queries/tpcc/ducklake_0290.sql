...
parameters AS (
	SELECT
		DATE '2026-06-16' AS start_date,
		DATE '2026-06-27' AS end_date,
		10::INTEGER AS lookback_days,
		'balanced' AS ready_status,
		'credit' AS review_status,
		'debt' AS blocked_status,
		'orders_4' AS order_bucket,
		'payments_4' AS payment_bucket,
		'stock_3' AS stock_bucket
),
date_spine AS (
		SELECT DATE '2026-06-16' AS report_as_of_date
		UNION ALL SELECT DATE '2026-06-17' AS report_as_of_date
		UNION ALL SELECT DATE '2026-06-18' AS report_as_of_date
		UNION ALL SELECT DATE '2026-06-19' AS report_as_of_date
		UNION ALL SELECT DATE '2026-06-20' AS report_as_of_date
		UNION ALL SELECT DATE '2026-06-21' AS report_as_of_date
		UNION ALL SELECT DATE '2026-06-22' AS report_as_of_date
		UNION ALL SELECT DATE '2026-06-23' AS report_as_of_date
		UNION ALL SELECT DATE '2026-06-24' AS report_as_of_date
		UNION ALL SELECT DATE '2026-06-25' AS report_as_of_date
		UNION ALL SELECT DATE '2026-06-26' AS report_as_of_date
		UNION ALL SELECT DATE '2026-06-27' AS report_as_of_date
),
stg_warehouse AS (
	SELECT
...
raw_order_activity AS (
	SELECT
		...
		CASE
			WHEN no.NO_O_ID IS NOT NULL THEN 'credit'
			WHEN o.O_CARRIER_ID IS NULL THEN 'debt'
			ELSE 'balanced'
		END AS order_status
	FROM dl.OORDER o
	...
	INNER JOIN date_spine ds
		ON CAST(o.O_ENTRY_D AS DATE) BETWEEN ds.report_as_of_date - INTERVAL 10 DAY
...
history_activity AS (
	...
	FROM dl.HISTORY h
	INNER JOIN date_spine ds
		ON CAST(h.H_DATE AS DATE) BETWEEN ds.report_as_of_date - INTERVAL 10 DAY
		AND ds.report_as_of_date
	GROUP BY
		ds.report_as_of_date,
		h.H_C_W_ID,
...
unpivoted AS (
	SELECT
		...
		'district_signal' AS metric_family,
	...
	SELECT
		...
		ec.district_name_norm,
		ec.customer_last_norm,
		'warehouse_signal' AS metric_family,
		CASE
			WHEN ec.payment_amount > ec.balance_amount THEN 'balanced'
			WHEN ec.payment_events > 0 THEN 'credit'
			ELSE 'debt'
	...
	SELECT
		...
		'item_signal' AS metric_family,
		CASE
			WHEN ls.stock_quantity > 60 THEN 'balanced'
			WHEN ls.stock_quantity > 40 THEN 'credit'
			ELSE 'debt'
		END AS metric_status,
		ls.item_id,
		ls.stock_quantity,
...
SELECT
	...
	CASE
		WHEN metric_amount >= 500.0 THEN 'large'
		WHEN metric_amount >= 5.0 THEN 'medium'
		ELSE 'small'
	END AS metric_size
FROM final_enriched
ORDER BY
