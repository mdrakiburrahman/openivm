-- {"complexity": "extreme", "is_incremental": true, "ducklake": true, "openivm_verified": true, "operators": "DUCKLAKE,CTE,MATERIALIZED_CTE,CTE_REF,SELF_JOIN,WINDOW,AGGREGATE,HAVING"}
WITH
parameters AS (
	SELECT
...
reuse_window_1 AS (
	SELECT
		...
		rs.item_id,
		rs.credit_code,
		rs.fulfillment_state,
		rs.account_state,
		rs.extended_amount,
		rs.customer_balance,
		rs.stock_quantity,
		rs.total_history_amount,
		rs.history_rows,
		rs.payment_history_rows,
		ROW_NUMBER() OVER (
			PARTITION BY rs.warehouse_id, rs.district_id, rs.customer_id
			ORDER BY rs.extended_amount DESC, rs.order_id ASC, rs.line_number ASC
		) AS customer_reuse_rank,
		SUM(rs.extended_amount) OVER (
			PARTITION BY rs.warehouse_id, rs.district_id, rs.customer_id
		) AS customer_reuse_amount
	FROM reuse_seed rs
...
reuse_window_2 AS (
	SELECT
		...
		rw1.item_id,
		rw1.credit_code,
		rw1.fulfillment_state,
		rw1.account_state,
		rw1.extended_amount,
		rw1.customer_balance,
		rw1.stock_quantity,
		rw1.total_history_amount,
		rw1.history_rows,
		rw1.payment_history_rows,
		rw1.customer_reuse_rank,
		rw1.customer_reuse_amount,
		DENSE_RANK() OVER (
			PARTITION BY rw1.warehouse_id, rw1.credit_code
			ORDER BY rw1.customer_reuse_amount DESC, rw1.customer_id ASC
		) AS credit_reuse_rank,
		COUNT(*) OVER (
			PARTITION BY rw1.warehouse_id, rw1.district_id, rw1.fulfillment_state
		) AS reuse_fulfillment_rows
	FROM reuse_window_1 rw1
...
reuse_left AS (
	SELECT
		...
		rw2.item_id,
		rw2.credit_code,
		rw2.fulfillment_state,
		rw2.extended_amount,
		rw2.customer_balance,
		rw2.stock_quantity,
		rw2.customer_reuse_rank,
		rw2.credit_reuse_rank,
		rw2.reuse_fulfillment_rows
	FROM reuse_window_2 rw2
	WHERE rw2.fulfillment_state IN ('open', 'delivered')
...
reuse_right AS (
	SELECT
		...
		rw2.credit_code,
		rw2.account_state,
		COUNT(*) AS customer_line_rows,
		SUM(rw2.extended_amount) AS customer_amount,
		SUM(rw2.total_history_amount) AS customer_history_amount,
		MAX(rw2.stock_quantity) AS max_customer_stock,
		MAX(rw2.credit_reuse_rank) AS max_credit_reuse_rank,
		COUNT(*) FILTER (WHERE rw2.payment_history_rows > 0) AS customer_payment_rows
	FROM reuse_window_2 rw2
	GROUP BY
		rw2.warehouse_id,
		rw2.district_id,
...
reuse_self_joined AS (
	SELECT
		rl.warehouse_id,
		rl.district_id,
		rl.credit_code,
		rr.account_state,
		rl.fulfillment_state,
		rl.customer_id,
		rl.order_id,
		rl.line_number,
		rl.item_id,
		...
		rl.customer_balance,
		rl.stock_quantity,
		rl.customer_reuse_rank,
		rl.credit_reuse_rank,
		rl.reuse_fulfillment_rows,
		rr.customer_line_rows,
		rr.customer_amount,
		rr.customer_history_amount,
		rr.max_customer_stock,
		rr.max_credit_reuse_rank,
		rr.customer_payment_rows
	FROM reuse_left rl
	JOIN reuse_right rr
		ON rr.warehouse_id = rl.warehouse_id
		AND rr.district_id = rl.district_id
		AND rr.customer_id = rl.customer_id
		AND rr.credit_code = rl.credit_code
...
reuse_rollup AS (
	SELECT
		...
		fulfillment_state,
		COUNT(*) AS reuse_rows,
		SUM(extended_amount) AS reuse_amount,
		SUM(customer_amount) AS reused_customer_amount,
		SUM(customer_history_amount) AS reused_history_amount,
		MIN(customer_balance) AS min_reuse_balance,
		MAX(stock_quantity) AS max_reuse_stock,
		MAX(max_customer_stock) AS max_customer_stock_seen,
		MAX(max_credit_reuse_rank) AS max_credit_reuse_rank,
		MAX(reuse_fulfillment_rows) AS max_reuse_fulfillment_rows,
		SUM(customer_payment_rows) AS reused_payment_rows
	FROM reuse_self_joined
	GROUP BY
		warehouse_id,
		district_id,
		credit_code,
...
SELECT
	...
	reuse_rows,
	reuse_amount,
	reused_customer_amount,
	reused_history_amount,
	min_reuse_balance,
	max_reuse_stock,
	max_customer_stock_seen,
	max_credit_reuse_rank,
	max_reuse_fulfillment_rows,
	reused_payment_rows
FROM reuse_rollup
ORDER BY warehouse_id, district_id, credit_code, account_state, fulfillment_state
