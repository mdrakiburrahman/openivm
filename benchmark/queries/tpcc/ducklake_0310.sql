-- {"complexity": "extreme", "is_incremental": true, "ducklake": true, "openivm_verified": true, "operators": "DUCKLAKE,CTE,WIDE_JOIN,INNER_JOIN,LEFT_JOIN,PROJECTION,FILTER"}
WITH
parameters AS (
	SELECT
...
wide_projection_input AS (
	SELECT
		...
		ff.customer_balance,
		ff.stock_quantity,
		ff.warehouse_ytd AS repeated_warehouse_ytd,
		ff.district_ytd AS repeated_district_ytd,
		ff.customer_discount AS repeated_customer_discount,
		ff.item_price AS repeated_item_price,
...
wide_projection_filtered AS (
	SELECT
		...
		customer_balance,
		stock_quantity,
		repeated_warehouse_ytd,
		repeated_district_ytd,
		repeated_customer_discount,
		repeated_item_price,
		repeated_stock_order_count,
		repeated_new_order_key,
		(extended_amount + repeated_item_price + repeated_customer_discount) AS wide_score
	FROM wide_projection_input
	WHERE credit_code IS NOT NULL AND fulfillment_state IN ('open', 'closed', 'new_order')
