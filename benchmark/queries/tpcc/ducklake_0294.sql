...
parameters AS (
	SELECT
		DATE '2026-06-15' AS start_date,
		DATE '2026-06-26' AS end_date,
		9::INTEGER AS lookback_days,
		'hot' AS ready_status,
		'warm' AS review_status,
		'cold' AS blocked_status,
		'orders_1' AS order_bucket,
		'payments_3' AS payment_bucket,
		'stock_1' AS stock_bucket
),
date_spine AS (
		SELECT DATE '2026-06-15' AS report_as_of_date
		UNION ALL SELECT DATE '2026-06-16' AS report_as_of_date
...
raw_order_activity AS (
	SELECT
		...
		CASE
			WHEN no.NO_O_ID IS NOT NULL THEN 'warm'
			WHEN o.O_CARRIER_ID IS NULL THEN 'cold'
			ELSE 'hot'
		END AS order_status
	FROM dl.OORDER o
	...
	INNER JOIN date_spine ds
		ON CAST(o.O_ENTRY_D AS DATE) BETWEEN ds.report_as_of_date - INTERVAL 9 DAY
...
history_activity AS (
	...
	FROM dl.HISTORY h
	INNER JOIN date_spine ds
		ON CAST(h.H_DATE AS DATE) BETWEEN ds.report_as_of_date - INTERVAL 9 DAY
		AND ds.report_as_of_date
	GROUP BY
		ds.report_as_of_date,
		h.H_C_W_ID,
...
unpivoted AS (
	...
	SELECT
		...
		ec.district_name_norm,
		ec.customer_last_norm,
		'pending' AS metric_family,
		CASE
			WHEN ec.payment_amount > ec.balance_amount THEN 'hot'
			WHEN ec.payment_events > 0 THEN 'warm'
			ELSE 'cold'
	...
	SELECT
		...
		'reserve' AS metric_family,
		CASE
			WHEN ls.stock_quantity > 64 THEN 'hot'
			WHEN ls.stock_quantity > 44 THEN 'warm'
			ELSE 'cold'
		END AS metric_status,
		ls.item_id,
		ls.stock_quantity,
...
SELECT
	...
	CASE
		WHEN metric_amount >= 900.0 THEN 'large'
		WHEN metric_amount >= 9.0 THEN 'medium'
		ELSE 'small'
	END AS metric_size
FROM final_enriched
ORDER BY
