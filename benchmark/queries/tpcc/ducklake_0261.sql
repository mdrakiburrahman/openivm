...
parameters AS (
	SELECT
		DATE '2026-06-03' AS start_date,
		DATE '2026-06-13' AS end_date,
		6::INTEGER AS lookback_days,
		'ready' AS ready_status,
		'review' AS review_status,
		'blocked' AS blocked_status,
		'orders_3' AS order_bucket,
		'payments_0' AS payment_bucket,
		'stock_4' AS stock_bucket
),
date_spine AS (
		SELECT DATE '2026-06-03' AS report_as_of_date
		UNION ALL SELECT DATE '2026-06-04' AS report_as_of_date
		UNION ALL SELECT DATE '2026-06-05' AS report_as_of_date
		UNION ALL SELECT DATE '2026-06-06' AS report_as_of_date
		UNION ALL SELECT DATE '2026-06-07' AS report_as_of_date
...
raw_order_activity AS (
	SELECT
		...
		CASE
			WHEN no.NO_O_ID IS NOT NULL THEN 'review'
			WHEN o.O_CARRIER_ID IS NULL THEN 'blocked'
			ELSE 'ready'
		END AS order_status
	FROM dl.OORDER o
	...
	INNER JOIN date_spine ds
		ON CAST(o.O_ENTRY_D AS DATE) BETWEEN ds.report_as_of_date - INTERVAL 6 DAY
...
history_activity AS (
	...
	FROM dl.HISTORY h
	INNER JOIN date_spine ds
		ON CAST(h.H_DATE AS DATE) BETWEEN ds.report_as_of_date - INTERVAL 6 DAY
		AND ds.report_as_of_date
	GROUP BY
		ds.report_as_of_date,
		h.H_C_W_ID,
...
unpivoted AS (
	...
	SELECT
		...
		ec.district_name_norm,
		ec.customer_last_norm,
		'payment_value' AS metric_family,
		CASE
			WHEN ec.payment_amount > ec.balance_amount THEN 'ready'
			WHEN ec.payment_events > 0 THEN 'review'
			ELSE 'blocked'
	...
	SELECT
		...
		'inventory_value' AS metric_family,
		CASE
			WHEN ls.stock_quantity > 65 THEN 'ready'
			WHEN ls.stock_quantity > 45 THEN 'review'
			ELSE 'blocked'
		END AS metric_status,
		ls.item_id,
		ls.stock_quantity,
