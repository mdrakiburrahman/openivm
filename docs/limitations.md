...
| `LATERAL` / correlated subquery aggregates represented as `DELIM_JOIN` / `DEPENDENT_JOIN` | Affected-key `GROUP_RECOMPUTE`: visible non-aggregate output columns are used as the recompute keys, then only those keys are deleted/reinserted from the view query. This is correct and incremental, but less precise than a fully algebraic correlated-aggregate delta. Exotic lateral projections where the visible non-aggregate columns are not the true correlation keys may need a more explicit delim-key extractor. |
