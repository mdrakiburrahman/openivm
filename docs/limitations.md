...
| Aggregates not in `SUPPORTED_AGGREGATES` (STRING_AGG, LISTAGG, GROUP_CONCAT, MEDIAN, percentiles, APPROX_*, QUANTILE_*, ANY_VALUE, ARG_MAX, ARG_MIN, …) | Order-dependent, holistic, or non-decomposable; no known delta formula. Supported set: `count_star, count, sum, min, max, avg, list, stddev, stddev_samp, stddev_pop, variance, var_samp, var_pop, bool_and, bool_or`. |
...
| `GROUPING SETS`, `ROLLUP`, `CUBE` | One `LogicalAggregate` produces rows for multiple grouping sets; the incremental aggregate path can only consume one set at a time. Plan §7 calls for exploding into a `UNION ALL` of N single-set aggregates; until that lands, routed to FULL_REFRESH (no empty-delta short-circuit). |
...
| `SELECT cols … ORDER BY col LIMIT k` (projection top-k, no GROUP BY) | Genuinely incremental: the `LIMIT`/`ORDER BY` is stripped from the data-table query and stored as the user-facing VIEW's `top_k_suffix`. The data table is maintained over the unlimited result via `SIMPLE_PROJECTION`, and read-time queries see only the top-k rows. See [operators/top-k.md](operators/top-k.md). |
| Inner `DISTINCT` directly inside a subquery feeding an outer `AGGREGATE` (e.g. `SELECT g, SUM(c) FROM (SELECT DISTINCT g, m, c FROM t) GROUP BY g`) | Group-level partial recompute (`IVMType::GROUP_RECOMPUTE`). For each base table with a non-empty delta, the LPTS view query is rewritten to scope that table to its delta-touched rows; we project `DISTINCT group_columns` from each variant, union across sources, and the resulting affected-keys set drives `DELETE` + `INSERT` on `_ivm_data_<mv>`. **Z-set-correct DBSP DISTINCT** (`distinct(R)[t] = sgn(R[t])`, Δdistinct fires only on count transitions across zero) requires per-tuple count auxiliary state; the partial-recompute path is the interim until that lands (see plan §5 in repo). |
