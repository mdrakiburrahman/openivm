...
ModifiedPlan IncrementalProjectionRule::Rewrite(PlanWrapper pw) {
	OPENIVM_DEBUG_PRINT("[IncrementalProjectionRule] Rewriting PROJECTION node, %zu expressions\n",
	...
	auto mul_expression =
	    make_uniq<BoundColumnRefExpression>(openivm::MULTIPLICITY_COL, pw.mul_type, child_mul_binding);
	...
	OPENIVM_DEBUG_PRINT(
	    "[IncrementalProjectionRule] Done, %zu expressions (including mul), mul_binding: table=%lu col=%lu\n",
